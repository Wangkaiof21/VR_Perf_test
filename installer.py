...
def help():
    ...
    OUT_MSG(
        "\tinstaller.py install: Install development environment (dependent, environment variables, etc.), From the KBE_ROOT search.")
...
def _checkKBEEnvironment(is_get_error):
    ...
    if not os.path.isdir(kbe_path):
        if is_get_error:
            ERROR_MSG(
                "KBE_ROOT: is error! The directory or file not found:\nKBE_ROOT/%s\nKBE_ROOT=%s" % (kbe_path, KBE_ROOT))
        return False
    ...
    if platform.system() == 'Windows':
        ...
        if name.lower() == 'uid':
            ...
            if uid != str(os.geteuid()):
                ret, cret = syscommand(
                    'bash -c \'usermod -d /home/%s/ -u %s %s\'' % (pwd.getpwnam(username).pw_dir, uid, username), True)
                INFO_MSG(ret)
                INFO_MSG(cret)
            return
    ...
    while True:
        ...
        if len(ret) == 0:
            if platform.system() == 'Windows':
                ...
                tmp = ""
                ret = subprocess.Popen("whereis mysql", shell=True, stdout=subprocess.PIPE,
                                       stderr=subprocess.STDOUT).stdout.readlines()[0].split()
                if len(ret) > 0:
        ...
        if len(mysql_root_password) == 0:
            ...
            cmd = "\"" + mysql_home + (
                    "mysql\" %s%s -hlocalhost -e" % (getRootOpt(mysql_root_password), rootusePortArgs)) + sql
            ...
            if len(ret) == 0:
                ...
                cmd = "\"" + mysql_home + ("mysql\" %s%s -hlocalhost -e" % (
                    getRootOpt(mysql_root_password), rootusePortArgs)) + sql + " mysql"
        ...
        if len(mysql_kbe_name) == 0:
            ...
            sql = "\"grant all privileges on *.* to %s@\'%%\' identified by \'%s\';grant select,insert,update,delete,create,drop on *.* to %s@\'%%\' identified by \'%s\';FLUSH PRIVILEGES\"" % (
                mysql_kbe_name, mysql_kbe_password, mysql_kbe_name, mysql_kbe_password)
            cmd = "\"" + mysql_home + ("mysql\" %s%s -hlocalhost -e" % (
                getRootOpt(mysql_root_password), rootusePortArgs)) + sql + " mysql"
            syscommand(cmd, False)
        ...
        cmd = "\"" + mysql_home + ("mysql\" -u%s -p%s -hlocalhost -P%s -e" % (
            mysql_kbe_name, mysql_kbe_password, mysql_port)) + has_db_sql
        ret, cret = syscommand(cmd, True)
        ...
        cmd = "\"" + mysql_home + (
                "mysql\" -u%s -p%s -hlocalhost -P%s -e" % (mysql_kbe_name, mysql_kbe_password, mysql_port)) + sql
        syscommand(cmd, False)
...
def getInstallPath():
    ...
    while True:
        ...
        if len(os_user_name) == 0:
            ...
            _install_path = getInput(
                "Please enter the installation path(No inout is %s):" % (pwd.getpwnam(os_user_name).pw_dir))
...
def copy_new_to_kbengine_dir(checksources=True):
    ...
    if os.path.isdir(KBE_ROOT):
        if os.path.isdir(kbe_res_path1):
            if getInput(
                    'Found that the existing directory(%s), whether to replace the: [yes|no]?' % kbe_res_path1) == "yes":
        ...
        if os.path.isdir(kbe_tools_path):
            if getInput(
                    'Found that the existing directory(%s), whether to replace the: [yes|no]?' % kbe_tools_path) == "yes":
        ...
        if checksources:
            ...
            if os.path.isdir(srcpath):
                if getInput('Found that the existing directory(%s), whether to replace the: [yes|no]?' % (
                        srcpath)) == "yes":
